...
from dataloader_dp import SummarizationDataLoader,SummarizationDataset
from evaluation import evaluate_const_tree_list
	...
	for i_data, test_inputs_dir in enumerate(test_inputs_dir_list):
		...
		for layer in [0,1]:
			for cons in ['nocons','sent_cons','sent_para_cons']:
				print('Dataset: %s, Model: %s, Layer use: %d, constraint type: %s'\
					%(dataset_list[i_data],args.model_name,layer,cons))
				sys.stdout.flush()	
				...
				#     mat=attn_list[i][1][0]
				#     const_tree_cand=build_const_tree_bottomup_greedy(mat,disco_to_sent_list[i])
					if cons=='nocons':
				...
				result_all['%d-%s'%(layer,cons)] = result
				evaluate_const_tree_list(const_tree_gt_list,const_tree_cand_list)
		save_path='./summ_guided_dp_result/%s-%s-const.pt'%(dataset_list[i_data],args.model_name)
